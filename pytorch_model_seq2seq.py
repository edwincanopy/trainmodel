...
class RnnLipModel(nn.Module):
    def __init__(self, input_dim=40, hidden_dim=20, output_dim=3, num_layers=4):
        ...
        self.encoder = nn.LSTM(input_dim, hidden_dim, num_layers, batch_first=True, bidirectional=True, dropout=0.5)
        self.decoder = nn.LSTM(output_dim, hidden_dim, num_layers, batch_first=True, bidirectional=True, dropout=0.5)
...
def main_train_resume():
    ...
    model = RnnLipModel().to(device)
    optimizer = optim.Adam(model.parameters(), lr=LEARNING_RATE)
    checkpoint = torch.load('rnn_model.pth')
...
def main_infer(weights_path, data_folder):
    model = RnnLipModel().to(device)
    model.load_state_dict(torch.load(weights_path, map_location=device))
    print(f'Loaded weights from {weights_path}')
    infer_all_data = get_infer_tuple(data_folder)

    preds = []
    for input_name, _ in infer_all_data:
        input = get_file(os.path.join(data_folder, input_name))
        input = torch.tensor(input, dtype=torch.float32).unsqueeze(0).to(device)
        pred = model(input).squeeze(0).cpu()
        preds.append(pred.tolist())

        with open(f'{data_folder}/{input_name[:-4]}_preds.txt', 'w') as f:
            f.write(str(pred.tolist()))

    with open('model_preds.txt', 'w') as f:
        f.write(str(preds))

    print('Saved model predictions in infer folder/')

if __name__ == '__main__':
    error_msg = "\nInvalid mode given. Modes are 'train', 'train_resume', or 'infer [folder]''"
    ...
    if len(sys.argv) == 1:
        ...
        if mode == 'train':
            ...
            main_infer(infer_weights_path, data_folder)
        else:
            print(error_msg)
